...
EDITOR_PROMPT = """You are a careful medical fact-checker. You are given a statement, the question that was checked, and an article snippet retrieved from the web.

Statement: {claim}
Question checked: {query}
Article found: {evidence}

Instructions:
- Rewrite the statement so that every fact it contains is directly supported by the article.
- Correct anything the article contradicts, and delete any detail the article does not support. Do NOT add facts, mechanisms, numbers, or explanations that are not explicitly stated in the article.
- Write the result as one clear, self-contained statement on the same topic and roughly the same length as the original.
- If the article already fully supports the statement, repeat it unchanged. If the article is irrelevant or lacks enough information, leave the statement unchanged.
Output ONLY the final statement on a single line, prefixed exactly with "My fix:" and nothing else.

My fix:""".strip()

CONTEXTUAL_EDITOR_PROMPT = """You are a careful medical fact-checker. You are given prior context, a statement, the question that was checked, and an article snippet retrieved from the web.
